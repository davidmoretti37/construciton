    ...
    try {
      ...
      const result = await onAction({
        ...
        data: {
          ...
          service_type: (current?.serviceType || current?.service_type || 'other').toLowerCase().replace(/\s+/g, '_'),
          billing_cycle: bc,
          ...
          address: current?.address || current?.location_address || current?.location || current?.locations?.[0]?.address || null,
          location_name: current?.location_name || current?.clientName || current?.client_name || null,
          location_address: current?.location_address || current?.address || current?.location || current?.locations?.[0]?.address || null,
