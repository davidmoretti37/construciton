...
export default function InvoicePreview({ data, onAction }) {
  ...
  // Defensive: `data` can come from cards generated by the AI (always shaped
  // correctly) or from the Invoices DB row (where `items` is JSONB and can
  // arrive as an array, a JSON-stringified array, null, or — for legacy
  // partial-payment invoices — missing entirely). Normalize to an array up
  // front so the rest of the component can rely on `.map()`.
  const safeData = data || {};
  const rawItems = safeData.items;
  const items = Array.isArray(rawItems)
    ? rawItems
    : (typeof rawItems === 'string'
        ? (() => { try { const p = JSON.parse(rawItems); return Array.isArray(p) ? p : []; } catch { return []; } })()
        : []);
