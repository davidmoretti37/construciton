...
const OwnerLumaBar = ({ state, navigation }) => {
  ...
  // Dynamic shimmer colors - Royal Blue theme for owner
  const shimmerColors = isDark
    ? [Colors.border, '#1E40AF', Colors.border, '#1E40AF', Colors.border]
    : ['#9CA3AF', '#60A5FA', '#1E40AF', '#60A5FA', '#9CA3AF'];
...
const NavItem = ({ item, isActive, onPress, Colors }) => {
  ...
  // Owner uses Royal Blue as primary color
...
const createStyles = (Colors) => StyleSheet.create({
  container: {
    // Positioning handled by parent OwnerNavContainer
    alignItems: 'center',
    justifyContent: 'center',
  },
  shimmerContainer: {
