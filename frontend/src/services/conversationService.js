...
export const sendManualMessage = async (projectId, message) => {
  try {
    ...
    if (!project.client_phone) {
      logger.error('No client phone number on project');
      return false;
    }
...
export const markConversationHandled = async (conversationId) => {
  try {
    ...
    if (error) {
      logger.error('Error marking conversation handled:', error);
      return false;
    }
...
export const getUnhandledConversationCount = async (projectId) => {
  try {
    ...
    if (error) {
      logger.error('Error getting unhandled count:', error);
      return 0;
    }

    return count || 0;
  } catch (error) {
    logger.error('Error getting unhandled conversation count:', error);
    return 0;
  }
};
