...
const calculateTimeBasedCompletion = (startDate, endDate) => {
  ...
  try {
    ...
    logger.error('❌ Error calculating time-based completion:', error);
    return 0;
  }
};
...
export const saveProject = async (projectData) => {
  try {
    ...
    logger.debug('✅ Project saved successfully:', result.id);

    // Save phases if provided
    if (projectData.phases && projectData.phases.length > 0) {
      logger.debug('💾 Saving project phases...');
...
export const calculateActualProgress = async (projectId) => {
  try {
    ...
    if (error || !phases || phases.length === 0) {
      logger.error('Error fetching phases for progress:', error);
      return 0;
    }
    ...
    logger.error('Error in calculateActualProgress:', error);
    return 0;
  }
};
...
export const calculateVelocity = async (projectId) => {
  try {
    ...
    if (projectError || !project) {
      logger.error('Error fetching project for velocity:', projectError);
      return 0;
    }
    ...
    if (phasesError || !phases) {
      logger.error('Error fetching phases for velocity:', phasesError);
      return 0;
    }
    ...
    logger.error('Error in calculateVelocity:', error);
    return 0;
  }
};
...
export const calculateEstimatedCompletion = async (projectId) => {
  try {
    ...
    if (projectError || !project) {
      logger.error('Error fetching project for completion estimate:', projectError);
      return null;
    }
    ...
    if (phasesError || !phases) {
      logger.error('Error fetching phases for completion estimate:', phasesError);
      return null;
    }
    ...
    logger.error('Error in calculateEstimatedCompletion:', error);
    return null;
  }
};
...
export const updateProjectProgress = async (projectId, actualProgress, isManual = false) => {
  try {
    ...
    logger.error('Error in updateProjectProgress:', error);
    return false;
  }
};
...
export const resetProjectProgressToAutomatic = async (projectId) => {
  try {
    ...
    logger.error('Error in resetProjectProgressToAutomatic:', error);
    return false;
  }
};
...
export const checkAndStartScheduledProjects = async () => {
  try {
    ...
    if (error) {
      logger.error('Error fetching scheduled projects:', error);
      return 0;
    }
    ...
    for (const project of projects) {
      ...
      logger.debug(`Auto-started project: ${project.name}`);
    }

    return projects.length;
  } catch (error) {
    logger.error('Error in checkAndStartScheduledProjects:', error);
    return 0;
  }
};
...
export const updateProjectPaymentStructure = async (projectId, paymentStructure, paymentTerms = null) => {
  try {
    ...
    logger.error('Error updating project payment structure:', error);
    return false;
  }
};
...
export const createProjectFromEstimate = async (estimateId) => {
  try {
    ...
    if (!estimate) {
      logger.error('Estimate not found');
      return null;
    }
    ...
    if (!createdProject) {
      logger.error('Failed to create project from estimate');
      return null;
    }
    ...
    if (updateError) {
      logger.error('Error updating estimate status:', updateError);
      // Don't fail the whole operation, project was created successfully
    }

    logger.debug('✅ Project created from estimate successfully:', createdProject.id);
...
export const fetchActiveProjectsForDate = async (date) => {
  try {
    ...
    logger.error('Error in fetchActiveProjectsForDate:', error);
    return [];
  }
};
