...
export default function PicturesScreen({ navigation }) {
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: Colors.background }]}>
        ...
        <View style={styles.filterSection}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.filterScroll}>
            ...
            {projectPhotoCounts.map(project => (
              ...
              >
                <Text style={[styles.filterChipText, { color: selectedProject === project.id ? '#FFFFFF' : Colors.primaryText }]}>
        ...
        {filteredPhotos.length === 0 ? (
          ...
          <View style={styles.photoGrid}>
            {filteredPhotos.map((photo) => (
              <TouchableOpacity
                key={photo.id}
                style={[styles.photoCard, { backgroundColor: Colors.cardBackground, borderColor: Colors.border }]}
                onPress={() => setSelectedPhoto(photo)}
                activeOpacity={0.8}
              >
                <Image source={{ uri: photo.uri }} style={[styles.photoImage, { backgroundColor: Colors.lightGray }]} />
      ...
      >
        <View style={[styles.modalContainer, { backgroundColor: isDark ? 'rgba(0, 0, 0, 0.98)' : 'rgba(0, 0, 0, 0.95)' }]}>
          ...
          {selectedPhoto && (
            <>
              ...
              <View style={styles.modalInfo}>
                <Text style={[styles.modalProjectName, { color: '#FFFFFF' }]}>{selectedPhoto.projectName}</Text>
                <Text style={[styles.modalDate, { color: '#CCCCCC' }]}>
