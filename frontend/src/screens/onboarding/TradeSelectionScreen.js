...
export default function TradeSelectionScreen({ navigation, route }) {
  ...
  const handleContinue = () => {
    if (selectedTrades.length === 0) {
      Alert.alert(t('tradeSelection.errors.selectService'), t('tradeSelection.errors.selectAtLeastOne'));
      return;
    }
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: Colors.background }]}>
      ...
      <View style={styles.header}>
        ...
        <Text style={[styles.headerTitle, { color: Colors.primaryText }]}>{t('tradeSelection.headerTitle')}</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      >
        <Text style={[styles.title, { color: Colors.primaryText }]}>
          {t('tradeSelection.title')}
        </Text>
        <Text style={[styles.subtitle, { color: Colors.secondaryText }]}>
          {t('tradeSelection.subtitle')}
        </Text>
          ...
          <View style={[styles.selectedBanner, { backgroundColor: Colors.success + '20' }]}>
            ...
            <Text style={[styles.selectedText, { color: Colors.success }]}>
              {selectedTrades.length > 1
                ? t('tradeSelection.servicesSelectedPlural', { count: selectedTrades.length })
                : t('tradeSelection.servicesSelected', { count: selectedTrades.length })}
            </Text>
          </View>
        )}
      </ScrollView>
