...
export default function SubscriptionSettingsScreen({ navigation }) {
  ...
  const getStatusBadge = () => {
    switch (status) {
      case 'trialing':
        return { color: Colors.primaryBlue, text: 'Trial' };
      case 'active':
        return { color: Colors.success, text: 'Active' };
      case 'past_due':
        return { color: Colors.warning, text: 'Past Due' };
      case 'canceled':
        return { color: Colors.error, text: 'Canceled' };
      default:
        return { color: Colors.secondaryText, text: 'Inactive' };
    }
  };
          ...
          {hasActiveSubscription && (
            <>
              ...
              <View style={styles.infoRow}>
                ...
                <Text style={[styles.infoText, { color: Colors.primaryText }]}>
                  {activeProjectCount} of{' '}
                  {projectLimit === 999999 ? 'unlimited' : projectLimit} projects used
                </Text>
              </View>
        ...
        {hasActiveSubscription ? (
          <>
            ...
            <Text style={[styles.helpText, { color: Colors.secondaryText }]}>
              {t(
                'subscription.manageBillingHelp',
                'Update payment method, view invoices, or cancel your subscription in the billing portal.'
              )}
            </Text>
          </>
        ) : (
          <>
            ...
            <Text style={[styles.helpText, { color: Colors.secondaryText }]}>
              {t(
                'subscription.noSubscriptionHelp',
                'Subscribe to access all features and manage more projects.'
              )}
            </Text>
          </>
        )}
