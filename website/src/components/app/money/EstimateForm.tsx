...
import type { DbEstimate, DbInvoiceLineItem } from "@/types/database";
...
export default function EstimateForm({ estimate, onSuccess, onCancel }: Props) {
  const isEdit = Boolean(estimate?.id);
  const router = useRouter();
  const { toast } = useToast();

  const action = isEdit
    ? updateEstimate.bind(null, estimate!.id)
    : createEstimate;
  ...
  useEffect(() => {
    if (state.ok && state.estimateId) {
      toast({
        title: isEdit ? "Estimate updated" : "Estimate created",
        variant: "success",
      });
      onSuccess?.(state.estimateId);
      router.refresh();
    } else if (state.error) {
    ...
    <form action={formAction} className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <Input
            ...
            invalid={Boolean(errs.estimate_number)}
            required
          />
          {errs.estimate_number && (
            <p className="text-[11px] text-[#c5251c] mt-1">{errs.estimate_number}</p>
          )}
        </div>
        <div>
          ...
          >
            {["draft", "sent", "viewed", "accepted", "declined", "expired", "converted"].map(
              (s) => (
      ...
      <div className="pt-2 border-t border-[#e5e5ea]">
        ...
        <Input
          ...
          step="0.01"
          min="0"
          defaultValue={String(estimate?.total ?? autoTotal.toFixed(2))}
          key={autoTotal}
      ...
      <div className="flex items-center justify-end gap-2 pt-1">
        ...
        <Button type="submit" variant="primary" size="md" disabled={pending}>
          {pending ? "Saving…" : isEdit ? "Save changes" : "Create estimate"}
        </Button>
      </div>
    </form>
  );
