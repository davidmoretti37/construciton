...
describe('GET /api/geocode', () => {
  ...
  test('missing GOOGLE_MAPS_API_KEY returns 500', async () => {
    ...
    const res = await request(app).get('/api/geocode').query({ address: '123 Main St' });
...
describe('GET /api/distance', () => {
  ...
  test('missing origins returns 400', async () => {
    const res = await request(app).get('/api/distance').query({ destinations: 'B' });
    expect(res.status).toBe(400);
    expect(res.body).toHaveProperty('error', 'Origins and destinations are required');
  });

  test('missing destinations returns 400', async () => {
    const res = await request(app).get('/api/distance').query({ origins: 'A' });
    expect(res.status).toBe(400);
    expect(res.body).toHaveProperty('error', 'Origins and destinations are required');
  });

  test('missing both params returns 400', async () => {
    const res = await request(app).get('/api/distance');
    expect(res.status).toBe(400);
    expect(res.body).toHaveProperty('error', 'Origins and destinations are required');
  });
  ...
  test('valid distance request returns data', async () => {
    ...
    const res = await request(app).get('/api/distance').query({ origins: 'A', destinations: 'B' });
    expect(res.status).toBe(200);
...
describe('GET /api/reverse', () => {
  ...
  test('missing lat returns 400', async () => {
    const res = await request(app).get('/api/reverse').query({ lng: '-74.006' });
    expect(res.status).toBe(400);
    expect(res.body).toHaveProperty('error', 'Latitude and longitude are required');
  });

  test('missing lng returns 400', async () => {
    const res = await request(app).get('/api/reverse').query({ lat: '40.7128' });
    expect(res.status).toBe(400);
    expect(res.body).toHaveProperty('error', 'Latitude and longitude are required');
  });

  test('missing both lat and lng returns 400', async () => {
    const res = await request(app).get('/api/reverse');
    expect(res.status).toBe(400);
    expect(res.body).toHaveProperty('error', 'Latitude and longitude are required');
  });
  ...
  test('valid coordinates returns address', async () => {
    ...
    const res = await request(app).get('/api/reverse').query({ lat: '40.7128', lng: '-74.006' });
    expect(res.status).toBe(200);
  ...
  test('no results returns 404', async () => {
    ...
    const res = await request(app).get('/api/reverse').query({ lat: '0', lng: '0' });
