...
// Verifies the request body SHA-256 matches the claim in the
// Plaid-Verification header JWT. Fetches the signing key from
// Plaid's API and validates using Node's built-in crypto.
// ============================================================
const verifyPlaidWebhook = async (req, res, next) => {
  const signedJwt = req.headers['plaid-verification'];
  if (!signedJwt) {
    logger.warn('Plaid webhook: missing Plaid-Verification header');
    return res.status(401).json({ error: 'Missing verification header' });
  }

  if (!plaidClient) {
    logger.warn('Plaid webhook: Plaid client not configured, cannot verify');
  ...
  try {
    ...
    // Fetch the signing key from Plaid
    ...
    const [, payloadB64, signatureB64] = signedJwt.split('.');
    const signatureInput = `${headerB64}.${payloadB64}`;
    const signature = Buffer.from(signatureB64, 'base64url');

    const isValid = crypto.verify(
      header.alg === 'ES256' ? 'SHA256' : 'SHA512',
    ...
    logger.error('Plaid webhook verification error:', error);
...
// ============================================================
router.post('/webhook', verifyPlaidWebhook, async (req, res) => {
  try {
