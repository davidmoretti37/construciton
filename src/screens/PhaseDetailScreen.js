...
export default function PhaseDetailScreen({ navigation, route }) {
  ...
  const getStatusIcon = (status) => {
    const statusMap = {
      ...
      'in_progress': { name: 'sync-circle', color: '#3B82F6' },
      'behind': { name: 'alert-circle', color: '#F59E0B' },
      'not_started': { name: 'pause-circle', color: '#6B7280' },
    };
              ...
              <View
                style={[
                  styles.progressBarFill,
                  {
                    ...
                    backgroundColor: progressPercent === 100 ? '#10B981' : Colors.primaryBlue,
                  }
                ]}
              />
            </View>
