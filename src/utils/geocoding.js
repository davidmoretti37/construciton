...
export const reverseGeocode = async (latitude, longitude) => {
  ...
  if (!GOOGLE_MAPS_API_KEY) {
    console.error('reverseGeocode: GOOGLE_MAPS_API_KEY not configured');
    return null;
  }
...
export const calculateTravelTime = async (origin, destination, departureTime = null) => {
  ...
  if (!GOOGLE_MAPS_API_KEY) {
    console.error('calculateTravelTime: GOOGLE_MAPS_API_KEY not configured');
    return null;
  }
  ...
  try {
    ...
    let url = `https://maps.googleapis.com/maps/api/distancematrix/json?origins=${encodeURIComponent(originStr)}&destinations=${encodeURIComponent(destStr)}&key=${GOOGLE_MAPS_API_KEY}&mode=driving`;
